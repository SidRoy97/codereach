...
export class CodeActionsProvider implements vscode.CodeActionProvider, vscode.Disposable {
  ...
  provideCodeActions(
    ...
    for (const diag of context.diagnostics) {
      ...
      if (!diag.source?.startsWith('Codescape')) continue;
  ...
  private async applyFix(doc: vscode.TextDocument, issue: Issue, _range: vscode.Range): Promise<void> {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'Codescape: Generating fix…', cancellable: false },
      async () => {
  ...
  private async showDiff(doc: vscode.TextDocument, before: string, after: string, line: number): Promise<void> {
    ...
    await vscode.commands.executeCommand(
      ...
      origUri,
      fixedUri,
      `Codescape Fix Preview — ${doc.fileName.split('/').pop()}`,
    );
  ...
  private async explainIssue(doc: vscode.TextDocument, issue: Issue): Promise<void> {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'Codescape: Generating explanation…', cancellable: false },
      async () => {
