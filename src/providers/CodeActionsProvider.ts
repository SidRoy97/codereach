...
export class CodeActionsProvider implements vscode.CodeActionProvider, vscode.Disposable {

  private disposables: vscode.Disposable[] = [];

  constructor(
    private readonly store:   IResultStore,
    private readonly scanner: AiScanner,
  ) {
    // Register the fix command — fires when user clicks "Apply Fix"
    this.disposables.push(
      vscode.commands.registerCommand(
        'codereach.fixWithAi',
        (doc: vscode.TextDocument, issue: Issue, range: vscode.Range) =>
          this.applyFix(doc, issue, range)
      )
    );
    ...
    this.disposables.push(
      vscode.commands.registerCommand(
        'codereach.explainIssue',
        (doc: vscode.TextDocument, issue: Issue) =>
          this.explainIssue(doc, issue)
      )
    );
  }
  ...
  provideCodeActions(
    ...
    for (const diag of context.diagnostics) {
      // Only handle diagnostics that CodeReach created
      if (!diag.source?.startsWith('CodeReach')) continue;
      ...
      const fix       = new vscode.CodeAction(
        `🔧 Fix: ${issue.message.slice(0, 55)}…`,
        vscode.CodeActionKind.QuickFix
      );
      fix.command     = { command: 'codereach.fixWithAi', title: 'Fix with AI', arguments: [document, issue, diag.range] };
      ...
      explain.command     = { command: 'codereach.explainIssue', title: 'Explain issue', arguments: [document, issue] };
  ...
  private async applyFix(doc: vscode.TextDocument, issue: Issue, _range: vscode.Range): Promise<void> {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'CodeReach: Generating fix…', cancellable: false },
      async () => {
        const fixed = await this.scanner.generateText(FIX_PROMPT, prompt);

        if (!fixed) {
          vscode.window.showWarningMessage('CodeReach: AI could not generate a fix.');
          return;
        }
        ...
        if (choice === 'Apply Fix') {
          ...
          vscode.window.showInformationMessage('CodeReach: ✅ Fix applied.');
  ...
  private async showDiff(doc: vscode.TextDocument, before: string, after: string, line: number): Promise<void> {
    ...
    await vscode.commands.executeCommand(
      ...
      origUri,
      fixedUri,
      `CodeReach Fix Preview — ${doc.fileName.split('/').pop()}`,
    );

    // Clean up the content provider after 30 seconds
    setTimeout(() => provider.dispose(), 30_000);
  }

  // Ask the AI to explain the issue, then show the explanation in a side panel
  private async explainIssue(doc: vscode.TextDocument, issue: Issue): Promise<void> {
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'CodeReach: Generating explanation…', cancellable: false },
      async () => {
        const text = await this.scanner.generateText(EXPLAIN_PROMPT, prompt);
        if (!text) {
          vscode.window.showWarningMessage('CodeReach: Could not generate explanation.');
          return;
        }
...
class ExplainPanel {
  ...
  static show(issue: Issue, markdown: string): void {
...
<body>
  ...
  private static mdToHtml(md: string): string {
    return md
      .replace(/^### (.+)$/gm, '<h3>$1</h3>')
      .replace(/^## (.+)$/gm,  '<h2>$1</h2>')
      .replace(/^# (.+)$/gm,   '<h1>$1</h1>')
