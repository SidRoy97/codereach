...
class FileSummarizer {
    ...
    async summarizeWorkspace() {
        ...
        if (!root)
            return new Map();
        let newCount = 0;
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'CodeReach: Summarizing files…', cancellable: true }, async (progress, token) => {
        ...
        this.saveCache();
        if (newCount > 0) {
            vscode.window.showInformationMessage(`CodeReach: Summarized ${newCount} file(s). Now run "CodeReach: Generate AI Context File".`);
        }
