...
class CodeActionsProvider {
    constructor(store, scanner) {
        ...
        this.disposables.push(vscode.commands.registerCommand('codereach.fixWithAi', (doc, issue, range) => this.applyFix(doc, issue, range)));
        // Register the explain command — opens a side panel
        this.disposables.push(vscode.commands.registerCommand('codereach.explainIssue', (doc, issue) => this.explainIssue(doc, issue)));
    }
    ...
    provideCodeActions(document, _range, context) {
        ...
        for (const diag of context.diagnostics) {
            ...
            if (!diag.source?.startsWith('CodeReach'))
                continue;
    ...
    async applyFix(doc, issue, _range) {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'CodeReach: Generating fix…', cancellable: false }, async () => {
            ...
            const choice = await vscode.window.showInformationMessage('CodeReach: Fix ready — apply it?', 'Apply Fix', 'Preview Diff');
            if (choice === 'Apply Fix') {
    ...
    async showDiff(doc, before, after, line) {
        ...
        await vscode.commands.executeCommand('vscode.diff', origUri, fixedUri, `CodeReach Fix Preview — ${doc.fileName.split('/').pop()}`);
    ...
    async explainIssue(doc, issue) {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'CodeReach: Generating explanation…', cancellable: false }, async () => {
...
class ExplainPanel {
    static show(issue, markdown) {
        ...
        if (this.panel) {
            ...
            this.panel = vscode.window.createWebviewPanel('codereach.explain', 'CodeReach: Explanation', vscode.ViewColumn.Beside, { enableScripts: false });
